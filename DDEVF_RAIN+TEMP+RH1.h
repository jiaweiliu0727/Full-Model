...
{
// DDEVF sets up model and calls ODE_SOLVER, returns Params->sim_results
...
double ave_R = Params->PARS[50+pop];     //Initial resting spore density in each pop
...
double temp_now;  //CK// used simplify decay functions
...
Params->INITS[0] = Params->PARS[30+pop];	// initS for this pop
Params->INITS[1] = 0.0;						// initV  SET TO 0 FOR FUNGUS ONLY MODEL!! (No virus)
Params->INITS[3] = ave_R;					// initR  //CK// changed to use R(0) for each pop
// END OF NEW FUNCTION //
...
double neo_v	= 7.0;			// latent period of neonates (days) FUNGUS ONLY MODEL! (This value not used here)
...
sim_results[0][0]=1.0;sim_results[0][1]=0.0;sim_results[0][2]=0.0;  //CK// Feral results for week 0.  No infections on day 0, apparently. Every individual is in class S.
...
if(R_start<1.0){        //JL: Usually R_start>1.0, which means the starting days of collection are in advance of resting spore blooming.
	...
	Params->nuR=(rain_P/(1+rain_P2*exp(-rain_P3*total_rainfall)) - rain_P/(rain_P2+1))*exp(rand_nuR[0]);

	sim_results[0][5]= Params->nuR*initR;  //Saving the resting spores on day 0
	...
	sim_results[0][6]= Params->nuR*initR;  //Saving the resting spores on day 0
}
else{sim_results[0][5]=0.0; sim_results[0][6]=0.0;}   //JL: If R_start>1.0, there will not be conidia and resting spore at the beginning of data collection (day 0).
...
double Vstart = ratio*initV;						// viral cadavers after infected neonates die. Not used here in the fungus only model.
...
double Vkill=0, Fkill=0;				// total individuals killed (used in calculating fraction infected)
...
while (t_0<MAXT3+h)	{    //CK// change MAXT to MAXT2 to let it go to the end of the experimental datasets?
	...
	// --------------------- resting spores done -------------------- //
	else if (timing[1]<timing[0] && timing[1]<day+1)	{    //In this case, timing[0] is already 999.9
		FlagR_end=1;
		t_next=timing[1];
		timing[0]=999.9;
		timing[1]=999.9;
	...
	while (t<t_next)	{
		y_ode[0]=S;	y_ode[m+1]=F;	Params->POPS[3]=R;   //The first equation is for S, and the (m+1)th is one for conidia
		//printf("pop:%d t=%f\t S=%4.3e\t V=%4.3e\t F=%4.3e\t R=%4.3e\n",pop,t,S,V,F,R);

		for (i=1;i<=gstepsF;i++)	{
			//y_ode[1+i]=E_V[i];
			y_ode[i]=E_F[i];        //Multiple exposed classes
		}
		y_ode[m+2]=Fkill;           //Infected cadavers
		...
		DDtemp_now = Params->WDATA[pop][line_ticker - 1][4]-10.0;  //CK// begin calculation of accumulated Degree Days (using average temperature)
		if(DDtemp_now<0.0){DDtemp_now=0.0;}
		DD10 = DD10 + DDtemp_now;			//CK// summing degree days over time
...
//printf("Start rain calc! day: %d\n", line_ticker-1);		getc(stdin);
		for (rain_day= (line_ticker - beta - theta - 1);rain_day <= line_ticker - theta -1;rain_day++)	{  //Calculating the accumulative rainfall in certain days
			total_rainfall = Params->WDATA[pop][rain_day][1]+total_rainfall;
//printf("line: %d\t accumulating rain: %lf\n", rain_day, total_rainfall);		getc(stdin);
		}

		nuR2=(rain_P/(1+rain_P2*exp(-rain_P3*total_rainfall)) - rain_P/(rain_P2+1))*exp(rand_nuR[(int)t]);   //JL: Calculating the resting spore transmission rate, related to accumulative rainfall
        //JL: The stochasticity differs every day. Here use (int)t to get the stochasticity of the day that time t is in.
		...
		//Params->nuR = (DD10/fourth_size)*rain_P2*exp(rain_P*total_rainfall*exp(rand_nuR[(int)t]));
		//nuR2 = rain_P2*exp(rain_P*total_rainfall*exp(rand_nuR[(int)t]));

		//if(Params->POPS[3] == 0.0 || total_rainfall==0.0){Params->nuR = 0.0; nuR2 = 0.0;}
...
//printf("pop: %d day: %d conidia: %f\n", pop, day, y_ode[7]*Params->nuF);
	...
	if ((day+1)%7==0)	{
		...
		//ConiBefore=y_ode[m+1]*Params->nuF;  //CK// Saving the conidia on the beginning day of the week
	...
	if (FlagWeek==1)	{           //The ending day of this week
		//printf("t=%f S=%e IV=%e IF=%e\n",t,S,IV,IF);
		if ((S+IF)==0)	{
			VirFI[week]=0;	FungFI[week]=0;
		}
		else					{
			FungFI[week] = IF/(S+IF);           // fraction infected by fungus at the end of each week
		}
		P[1] = 1-(FungFI[week]);             //
		...
		sim_results[week][6]=Params->POPS[3]*nuR2;  //Saving the resting spores on the ending day of each week
